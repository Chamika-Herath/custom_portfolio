    ...
    <script>
        ...
        function checkPasswordStrength() {
            ...
            // Safely fetch checklist elements
            const upCheck = document.getElementById('uppercase-check');
            const lowCheck = document.getElementById('lowercase-check');
            const numCheck = document.getElementById('number-check');
            const specCheck = document.getElementById('special-check');
            const lenCheck = document.getElementById('length-check');

            // Reset checks safely
            if (upCheck) upCheck.style.color = 'var(--erp-text-tertiary)';
            if (lowCheck) lowCheck.style.color = 'var(--erp-text-tertiary)';
            if (numCheck) numCheck.style.color = 'var(--erp-text-tertiary)';
            if (specCheck) specCheck.style.color = 'var(--erp-text-tertiary)';
            if (lenCheck) lenCheck.style.color = 'var(--erp-text-tertiary)';
            ...
            // Update check icons safely
            if (hasUpperCase && upCheck) upCheck.style.color = 'var(--erp-accent-success)';
            if (hasLowerCase && lowCheck) lowCheck.style.color = 'var(--erp-accent-success)';
            if (hasNumbers && numCheck) numCheck.style.color = 'var(--erp-accent-success)';
            if (hasSpecialChar && specCheck) specCheck.style.color = 'var(--erp-accent-success)';
            if (hasMinLength && lenCheck) lenCheck.style.color = 'var(--erp-accent-success)';
        ...
        // NOTE: The rogue duplicate submit listener lacking e.preventDefault() was removed to allow the primary `onsubmit="return User_Registration_A_01_from_01_SUMBIT(event)"` to own the submission flow safely.
    </script>
